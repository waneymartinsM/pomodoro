...
import 'package:pomodoro/app/utils/constants/date_time_constants.dart';
import 'package:pomodoro/app/utils/extensions/extensions.dart';
import 'package:pomodoro/app/utils/helpers/helper_methods.dart';
import 'package:shamsi_date/shamsi_date.dart';

class EnglishDatePickerController extends GetxController
    implements CustomDatePickerController {
  late int _month;
  late int _year;
  ...
  void onWeekChanged(bool forward) {
    ...
    _currentWeek =
        currentWeek > maxNumberOfWeek ? kMinNumberOfWeek : maxNumberOfWeek;
  ...
  List<Date> getDaysOfWeek(int week) {
    ...
    _getWeek(
      ...
      onFind: (index, date) {
        beginIndex = index;
        endIndex =
            DateTime.daysPerWeek - (date.weekDay == 7 ? 0 : date.weekDay);
        endIndex += beginIndex;
        endIndex =
            endIndex >= daysOfMonth.length ? daysOfMonth.length : endIndex;
      },
    );
  ...
  int? _getWeek(
    ...
    for (int i = 0; i < daysOfMonth.length; i++) {
      ...
      final condition =
          date == null ? week == currentWeek : daysOfMonth[i].day == date.day;
      if (condition) {
